...
import { LazyResultadosParciais as ResultadosParciais } from '@/components/dynamic';
...
import { Plus, Vote, Lock } from 'lucide-react';
...
export default function VotosPage() {
  ...
  const { usuario, isParticipante, isAdmin } = useAuth();
  ...
  const [sessaoVotadaId, setSessaoVotadaId] = useState<number | null>(null);
  ...
  const handleSalvar = async (data: FormValues) => {
    ...
    try {
      ...
      setSessaoVotadaId(data.sessao_id);
      toast.success('Voto registrado com sucesso! Você já pode ver os resultados.');
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="px-6 space-y-6">
        {isParticipante && sessaoVotadaId && (
          <ResultadosParciais sessao_id={sessaoVotadaId} />
        )}
        {isParticipante && (
