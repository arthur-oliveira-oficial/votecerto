...
import { Pencil, Trash2, Search, FolderKanban, DollarSign, Eye } from 'lucide-react';
...
export default function ProjetosPage() {
  ...
  const [projetoParaExcluir, setProjetoParaExcluir] = useState<{ id: number; titulo: string } | null>(null);
  ...
  const handleExcluir = async () => {
    ...
    try {
      await excluir.trigger(projetoParaExcluir.id);
  ...
  const confirmarExclusao = (id: number, titulo: string) => {
    setProjetoParaExcluir({ id, titulo });
  };
      ...
      <AlertDialog open={!!projetoParaExcluir} onOpenChange={(open) => !open && setProjetoParaExcluir(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tem certeza que deseja excluir &quot;{projetoParaExcluir?.titulo}&quot;? Esta ação não pode ser desfeita.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
