...
import { Plus, Pencil, Trash2, Calendar, Clock, Users, FolderKanban, CheckCircle } from 'lucide-react';
...
export default function SessoesAdminPage() {
  ...
  const [sessaoParaExcluir, setSessaoParaExcluir] = useState<{ id: number; titulo: string } | null>(null);
  ...
  const handleExcluir = async () => {
    ...
    try {
      await excluir.trigger(sessaoParaExcluir.id);
  ...
  const confirmarExclusao = (id: number, titulo: string) => {
    setSessaoParaExcluir({ id, titulo });
  };
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {sessoes.map((sessao) => (
              <SessaoCardMemo
                ...
                onExcluir={() => confirmarExclusao(sessao.id, sessao.titulo)}
              />
            ))}
          </div>
        )}
      ...
      />
      <AlertDialog open={!!sessaoParaExcluir} onOpenChange={(open) => !open && setSessaoParaExcluir(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Confirmar exclusão</AlertDialogTitle>
            <AlertDialogDescription>
              Tem certeza que deseja excluir &quot;{sessaoParaExcluir?.titulo}&quot;? Esta ação não pode ser desfeita.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
