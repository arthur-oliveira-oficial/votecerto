...
import { Plus, Users, CheckCircle, DoorOpen, CopyIcon, Pencil, Trash2, RefreshCw } from 'lucide-react';
        ...
        {!isAdmin && (
          <div className="flex items-center gap-2 p-3 bg-slate-50 rounded-lg">
            <div className="flex-1">
              ...
              <p className="font-mono font-bold text-lg tracking-widest uppercase">
                {comunidade.codigo}
              </p>
            </div>
            <Button
              variant="ghost"
              size="icon"
              onClick={() => onRegenerarCodigo(comunidade.id)}
            ...
            <Button
              variant="ghost"
              size="icon"
              onClick={() => onCopiarCodigo(comunidade.codigo)}
              title="Copiar código"
            >
...
export default function ComunidadesPage() {
  const { comunidades: comunidadesDoUsuario, isLoading, mutate: mutateMinhas } = useMinhasComunidades();
  const { comunidades: todasComunidades, criar, atualizar, excluir, regenerarCodigo, isLoading: loadingTodas } = useComunidades();
  ...
  const handleRegenerarCodigo = async (id: number) => {
    try {
      await regenerarCodigo.trigger(id);
      await mutateMinhas();
